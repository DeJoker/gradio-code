...
with gr.Blocks(title="concat string", css=consts.css) as demo:
    step1_tb = gr.Textbox(label="step1")
    step2_tb = gr.Textbox(label="step2")
    step_out_tb = gr.Textbox(label="output_demo")

    add_btn = gr.Button("direct concat", variant="primary", elem_classes=["tool"])
    ...
    add_and_check_btn = gr.Button("both step exist then concat", variant="secondary", elem_classes=["tool"])
